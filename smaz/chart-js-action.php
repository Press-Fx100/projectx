...
<script>
    ...
    //bar chart
    let cawangan = "cawangan=<?php echo $_GET['cawangan'];?>";
    let controller = "controller=<?php echo $_GET['controller'];?>";
    
    let xhr = new XMLHttpRequest();
    xhr.open('POST', 'chart-js-action-queries.php');
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.send(cawangan + "&" + controller);
      ...
      if(xhr.readyState == 4 && xhr.status == 200) {
        ...
        let chart_settings = {
          ...
          options: {
            ...
            onClick:function (e) {
              var actions = this.getElementsAtEvent(e)[0]._model.label;
              location.href = "report-action-table.php?controller=<?php echo $_GET['controller']?>&actions="+actions+"&cawangan=<?php echo $_GET['cawangan']?>";
            }
          }
        }
    ...
    function tabletimeline() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $( '#table-action' ).html(response);
            }
        }); 
        $.ajax({
            type: 'post',
            url: 'timeline-default-action.php',
            data: {
                cawangan: "<?php echo $_GET['cawangan'] ?>",
                controller: "<?php echo $_GET['controller'] ?>",
            },
